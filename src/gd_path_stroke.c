...
static void reverse_gdpath(gdPathPtr path)
{
    if (!path)
        return;

    int numElements = gdArrayNumElements(&path->elements);
    int numPoints = gdArrayNumElements(&path->points);

    if (numElements == 0)
        return;

    gdPathOps *elements = (gdPathOps *)gdArrayGetData(&path->elements);
    gdPointF *points = (gdPointF *)gdArrayGetData(&path->points);

    gdPathPtr reversed = gdPathCreate();
    if (!reversed)
        return;

    int point_idx = numPoints - 1;
    ...
    for (int i = numElements - 1; i >= 0; i--)
    {
        switch (elements[i])
        {
            case gdPathOpsMoveTo:
                if (point_idx >= 0)
                    gdPathMoveTo(reversed, points[point_idx].x, points[point_idx].y);
                point_idx--;
                break;
            case gdPathOpsLineTo:
                if (point_idx >= 0)
                    gdPathLineTo(reversed, points[point_idx].x, points[point_idx].y);
                point_idx--;
                break;
            case gdPathOpsQuadTo:
                if (point_idx >= 1)
                {
                    double x2 = points[point_idx].x;
                    double y2 = points[point_idx].y;
                    double x1 = points[point_idx - 1].x;
                    double y1 = points[point_idx - 1].y;
                    gdPathQuadTo(reversed, x1, y1, x2, y2);
                    point_idx -= 2;
                }
                break;
            case gdPathOpsCubicTo:
                if (point_idx >= 2)
                {
                    double x3 = points[point_idx].x;
                    double y3 = points[point_idx].y;
                    double x2 = points[point_idx - 1].x;
                    double y2 = points[point_idx - 1].y;
                    double x1 = points[point_idx - 2].x;
                    double y1 = points[point_idx - 2].y;
                    gdPathCurveTo(reversed, x1, y1, x2, y2, x3, y3);
                    point_idx -= 3;
                }
                break;
            case gdPathOpsClose:
                gdPathClose(reversed);
                break;
        }
    }

    // Swap internal arrays instead of copying struct
    gdArray tmp_elements = path->elements;
    gdArray tmp_points = path->points;
    path->elements = reversed->elements;
    path->points = reversed->points;
    reversed->elements = tmp_elements;
    reversed->points = tmp_points;
    reversed->contours = path->contours;  // also swap contours
    path->contours = reversed->contours;

    gdPathDestroy(reversed);  // This destroys the OLD arrays (now in reversed)
}
