...
static int tga_read_color_map(gdIOCtx *ctx, oTga *tga)
{
    ...
    for (i = 0; i < tga->colormaplength; i++) {
        ...
        tga->colormap[i] =
            tga_decode_color(buf, tga->colormapbits, tga->colormapbits == 32 ? 8 : 0, &has_alpha);
    }
...
static void tga_apply_attribute_type(gdIOCtx *ctx, oTga *tga, int pixel_count)
{
    static const unsigned char signature[18] = {'T', 'R', 'U', 'E', 'V', 'I', 'S', 'I', 'O',
                                                'N', '-', 'X', 'F', 'I', 'L', 'E', '.', '\0'};
    ...
    if (trailing_size > (size_t)INT_MAX - (size_t)start ||
        attr_type_offset >= (uint32_t)start + (uint32_t)trailing_size - sizeof(footer)) {
        return;
    }
...
static int tga_decode_color(const unsigned char *buf, int bits, int alpha_bits, int *has_alpha)
{
    ...
    case 15:
    case 16:
        return tga_decode_16((unsigned int)buf[0] | ((unsigned int)buf[1] << 8), alpha_bits,
                             has_alpha);
...
static void tga_strip_alpha(oTga *tga, int pixel_count)
{
    ...
    for (i = 0; i < pixel_count; i++) {
        ...
        tga->bitmap[i] = gdTrueColor(gdTrueColorGetRed(pixel), gdTrueColorGetGreen(pixel),
