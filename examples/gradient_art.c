...
static int save_png(gdSurfacePtr surface, const char *name)
{
    FILE *fp = fopen(name, "wb");
    if (!fp) { fprintf(stderr, "Cannot write %s\n", name); return 0; }
    gdSurfacePng(surface, fp); fclose(fp); return 1;
}
...
int main(void)
{
    ...
    gdSurfacePtr surface=gdSurfaceCreate(w,h,GD_SURFACE_ARGB32);
    gdContextPtr c;
    gdPaintPtr p;
    unsigned int seed=0x51a7u;
    if(!surface) return 1;
    c=gdContextCreate(surface); if(!c){gdSurfaceDestroy(surface);return 1;}
    ...
    if(!save_png(surface,"gradient_art.png")){gdContextDestroy(c);gdSurfaceDestroy(surface);return 1;}
    gdContextDestroy(c); gdSurfaceDestroy(surface);
