 ...
 * This test requires a set of PNG images that have been losslessly encoded to
 * AVIFs. For each such image, we encode the PNG into an AVIF, with the GD
 * format as an intermediary, then compare the resulting AVIF with the original
 * PNG.
 *
 * We then do this process in reverse, encoding the AVIF into a PNG,
 * and compare the resulting PNG with the original AVIF.
 *
 * We report any discrepancies in the images, or any other errors that may
 * occur.
 */
...
int main() {
	...
	static const char *const filenames[] = {"baboon", "dice_with_alpha",
											"plum_blossom_12bit", "sunset"};
	const int filesCount = sizeof(filenames) / sizeof(filenames[0]);

	for (int i = 0; i < filesCount; i++) {

		// First, encode each PNG into an AVIF (with the GD format as an
		// intermediary), then compare the result with the original PNG.
		...
		strcat(strcpy(errMsg, filenames[i]),
			   ".png: gdImageCreateFromPng failed\n");
